...
export default function HighlightsView({ feeds }: Props): JSX.Element {
  ...
  const chipClass = (active: boolean): string =>
    cn(
      'inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded',
      active
        ? 'bg-bg-alt text-fg shadow-[inset_0_-2px_0] shadow-underline'
  ...
  return (
    <div className="max-w-[760px] mx-auto px-14 pt-14 pb-20">
      ...
      <div className="flex flex-col gap-3.5 mb-8 pb-4 border-b border-border">
        <Input
          type="text"
          placeholder={t('highlights.searchPlaceholder')}
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="border-0 border-b border-border rounded-none px-0 focus-visible:ring-0 focus-visible:border-accent"
        />
        <div className="flex flex-wrap gap-1.5">
          <Button
            ...
            className={chipClass(filterFeed === null)}
          >
        ...
        {filtered.map((e, i) => {
          return (
            <li
              ...
              className="cursor-pointer pl-4 border-l-2 border-fg-faint transition-transform duration-150 hover:translate-x-0.5"
            >
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2 label">
                <span className="font-semibold text-fg-dim">{feedTitle(e.feed)}</span>
